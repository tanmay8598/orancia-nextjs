...
const MobileBanner = ({ initialBanner = [] }) => {
    ...
    const SkeletonLoader = () => (
        <div className="relative w-full h-[150px] bg-gray-200 rounded-lg overflow-hidden">
                ...
                <>
                    <div className="relative w-full overflow-hidden">
                        {banner.map((item, index) => (
                            <div
                                key={item?._id}
                                className={`w-full transition-transform duration-500 ease-in-out ${index === currentIndex ? "translate-x-0" : "translate-x-full"
                                    }`}
                            >
                                <Image
